  ...
  function retryButtonNear(node) {
    ...
    for (let depth = 0; root && depth < 7; depth += 1, root = root.parentElement) {
      ...
      for (const button of buttons) {
        const label = `${button.innerText || button.textContent || ''} ${button.getAttribute('aria-label') || ''}`.replace(/\s+/g, ' ').trim();
        if (RETRY_TEXT.test(label) && !button.disabled && button.getAttribute('aria-disabled') !== 'true') return button;
      }
    }
    return null;
  }
