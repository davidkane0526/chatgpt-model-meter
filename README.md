...
- 默认开启**页面连接保活**：每 4 分钟执行一次轻量同源会话检查；页面从后台恢复时，如果已超过保活周期，会立即补一次检查。该功能只能降低长时间闲置后前端连接失效的概率，不能保证服务端永不超时。
...
- 设置仍保存到现有 `chrome.storage.sync` 配置中，不新增 Cookie、debugger、历史记录等权限。
