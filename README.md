# 模型 · 额度监测 for ChatGPT v0.9.0-mod

> 基于 `pfz14/chatgpt-model-meter` v0.7.4 修改。非官方第三方扩展，与 OpenAI 无隶属、合作或官方认可关系。

在 chatgpt.com 左上角显示 Work / Codex 的 5 小时、7 天剩余额度，并显示请求模型与服务端 STE 遥测报告的模型标识。v0.9.0 新增页面连接保活与“消息发送超时，请重试”自动恢复。

## v0.9.0：连接保活与发送超时自动恢复

- 新增独立 `resilience.js` 模块，不修改现有模型识别与额度读取核心逻辑。
- 默认开启**页面连接保活**：每 4 分钟执行一次轻量同源会话检查；页面从后台恢复时，如果已超过保活周期，会立即补一次检查。当前正在生成内容时跳过保活请求，避免干扰活动请求。该功能只能降低长时间闲置后前端连接失效的概率，不能保证服务端永不超时。
- 默认开启**发送超时自动恢复**，识别中文“消息发送超时，请重试”以及常见英文等价提示。
- 检测到超时后默认等待 2 秒，并优先寻找 ChatGPT 原生“重试 / Retry / Try again”按钮，存在时先重试原消息。
- 如果没有可用的原生重试按钮，则在输入框为空、且当前未生成内容时写入并发送默认恢复消息 `继续`。
- 如果用户正在输入内容，自动恢复会暂停，不覆盖输入框。
- 默认最多连续恢复 3 次；15 秒冷却防止 React/DOM 重绘造成重复触发。连续恢复计数在稳定 60 秒后自动清零。
- 每次恢复操作后 20 秒复检错误状态，避免原生“重试”被点击但错误仍残留。
- 切换到另一条 ChatGPT 对话后自动清空本轮恢复状态。
- 设置面板新增“连接稳定性”区域，可开关保活与自动恢复，并修改恢复消息、恢复延迟和最大连续恢复次数。
- 恢复模块使用独立的 `yyChatgptResilienceSettings` 键保存到 `chrome.storage.sync`，避免现有显示设置保存时相互覆盖；不新增 Cookie、debugger、历史记录等权限。
...
如果已经加载了旧版开发者扩展，拉取新版本后在 `chrome://extensions/` 中点击一次“重新加载”，然后刷新 ChatGPT 页面即可。
